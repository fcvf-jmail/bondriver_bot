...
module.exports = new Scenes.WizardScene("addReportScene",
    ...
    async ctx => {
        ...
        await ctx.reply("Укажите номер бытовки", { reply_markup: { inline_keyboard: [[cancelButton]], resize_keyboard: true } })
        return ctx.wizard.next()
    },
    async ctx => {
        if (ctx?.callbackQuery?.data == "cancelAdding") return await cancelAdding(ctx);
        ctx.scene.session.state.cabinNumber = ctx.message.text
...
async function generateFolderName(ctx) {
    var folderName = `/Доставки и вывозы/${ctx.scene.session.state.date}|${ctx.scene.session.state.cabinNumber}|${ctx.scene.session.state.nameOfCounteragent}|${ctx.from.first_name}${ctx.from.username ? ` @${ctx.from.username}` : ""}`
